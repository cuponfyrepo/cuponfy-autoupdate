...
class WPCoupon_Headline_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'st_headline', // Base ID
            esc_html__( 'Cuponfy Cabeçalho Customizado', 'wp-coupon' ), // Name
            array(
                'description' => esc_html__( 'Insira um cabeçalho customizado', 'wp-coupon' ),
    ...
    public function form( $instance ) {
        ...
        <p class="st-widget-sidebar-settings">
            <label for="<?php echo $this->get_field_id( 'tag' ); ?>"><?php esc_html_e( 'Nível de Cabeçalho', 'wp-coupon' ); ?></label>
            <select name="<?php echo $this->get_field_name( 'tag' ); ?>">
                <?php
                for ( $i = 1;  $i <= 6; $i++ ) {
                    echo '<option value="h'.$i.'" '.selected( $instance['tag'], 'h'.$i, false ).' >'.sprintf( 'H%s', $i ).'</option>';
                } ?>
            </select>
        </p>
        <?php
